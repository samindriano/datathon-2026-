"""Run the final fixed Task 1 city-distribution state experiment."""
...
from distribution_state_model import FEATURE_NAMES, fit_distribution_state_ridge
...
EXPERIMENT_ID = "d1-e014-diststate"
...
def acceptance_gate(candidates: dict, stress: dict, diagnostics: dict) -> dict:
    ...
    candidate = candidates["distblend"]
    ...
    checks = {
        ...
        "test_m2_correction_is_nonpositive": float(
            diagnostics["distblend_m2_mean_correction_vs_ridge"]
        )
        <= 0.0,
        "test_change_vs_e013_is_conservative": float(
            diagnostics["distblend_vs_e013_rms"]
        )
        <= 1.0,
...
def evaluate_candidates(blocks, text_blocks, adjacency, fold_sets, args):
    methods = ("stableblend", "diststate", "distblend")
    ...
    for block, texts, folds in zip(blocks, text_blocks, fold_sets, strict=True):
        ...
        for fold in folds:
            ...
            dist_model = fit_distribution_state_ridge(
                block, fold.train_origin_start, fold.train_origin_end,
                alpha=args.alpha, chunk_size=args.chunk_size,
            )
            ...
            dist_prediction = dist_model.predict(histories)
            candidate_prediction = stable_blend_predictions(
                anchor_prediction, dist_prediction
            )
            predictions = {
                "stableblend": e013_prediction,
                "diststate": dist_prediction,
                "distblend": candidate_prediction,
            }
    ...
    candidate_cells = np.asarray(candidates["distblend"]["block_fold_horizon_mse"])
...
def main() -> int:
    ...
    dist_models = [
        fit_distribution_state_ridge(block, 14, len(block) - int(HORIZONS.max()) - 1,
                                     alpha=args.alpha, chunk_size=args.chunk_size)
        for block in blocks
    ]
    text_models = [
    ...
    dist_prediction = np.empty(shape, dtype=np.float32)
    for index, (graph_model, global_model, dist_model, text_model) in enumerate(
        zip(graph_models, global_models, dist_models, text_models, strict=True)
    ):
        mask = regimes == index
        ...
        dist_prediction[mask] = dist_model.predict(histories)
    e013_prediction = stable_blend_predictions(anchor_prediction, global_prediction)
    candidate_prediction = stable_blend_predictions(anchor_prediction, dist_prediction)
    ...
    diagnostics = {
        "distblend_m2_mean_correction_vs_ridge": float(
            (candidate_prediction[m2] - ridge_prediction[m2]).mean()
        ),
        "distblend_vs_e013_rms": float(np.sqrt(np.mean(np.square(delta)))),
        "distblend_vs_e013_mean": float(delta.mean()),
        "distblend_vs_e013_min": float(delta.min()),
        "distblend_vs_e013_max": float(delta.max()),
    ...
    summary = {
        ...
        "hypothesis": "City dispersion and slow-road fractions add robust congestion-shape information beyond city mean under the unchanged 75:25 anchor blend.",
        "preregistered_parameters": {
            "features": list(FEATURE_NAMES),
            "anchor_weight": ANCHOR_WEIGHT,
            "diststate_weight": GLOBALSTATE_WEIGHT,
            "slow_thresholds_kmh": [30.0, 50.0],
            "alpha": args.alpha,
            "temporal_chunk_size_origins": TEMPORAL_CHUNK_SIZE,
            "seed": None,
        },
