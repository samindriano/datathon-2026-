"""Run d1-e009-textzguard on frozen folds and inference-shift gates."""
...
EXPERIMENT_ID = "d1-e009-textzguard"
...
def acceptance_gate(
    ...
    guarded_folds = np.asarray(guarded["fold_mse"], dtype=np.float64)
    ridge_horizons = np.asarray(list(ridge["mse_by_horizon"].values()))
    guarded_horizons = np.asarray(list(guarded["mse_by_horizon"].values()))
    checks = {
        "mean_improves_ridge_by_at_least_0_5_percent": float(guarded["mean_mse"])
        ...
        >= 2,
        "worst_fold_within_one_percent_of_ridge": float(guarded["worst_fold_mse"])
        <= 1.01 * float(ridge["worst_fold_mse"]),
        "mean_within_0_1_percent_of_textres": float(guarded["mean_mse"])
        <= 1.001 * float(textres["mean_mse"]),
        "guard_activates_on_test_m2": int(
            diagnostics["test_m2_guarded_samples"]
        )
        > 0,
        "test_m2_mean_correction_is_nonpositive": float(
            diagnostics["guarded_m2_mean_correction_vs_ridge"]
        )
        <= 0.0,
        "test_m2_correction_reduced": float(
            diagnostics["guarded_m2_mean_correction_vs_ridge"]
        )
        < float(diagnostics["exact_m2_mean_correction_vs_ridge"]),
    }
    return {"checks": checks, "status": "KEEP" if all(checks.values()) else "REJECT"}


def evaluate_candidates(blocks, text_blocks, fold_sets, args):
    methods = ("ridge-history", "textres", "textzguard")
    ...
    for block, texts, folds in zip(blocks, text_blocks, fold_sets, strict=True):
        ...
        for fold in folds:
            ...
            predictions = {
                ...
                "textzguard": model.predict(histories, validation_texts),
            }
            block_guards.append(int(model.guard_mask(validation_texts).sum()))
            for method in methods:
                block_scores[method].append(
                    mse_by_horizon(targets, predictions[method])
                )
        guard_counts.append(block_guards)
        for method in methods:
...
def main() -> int:
    ...
    exact_prediction = np.empty(shape, dtype=np.float32)
    guarded_prediction = np.empty(shape, dtype=np.float32)
    test_guard_counts = {}
    ...
    summary = {
        ...
        "hypothesis": "A fixed three-sigma guard removes the audited text OOD path while preserving aligned-text gain.",
        "preregistered_parameters": {
            "base_alpha": args.base_alpha,
            "residual_alpha": args.residual_alpha,
            "guarded_feature": GUARDED_FEATURE,
            "guard_trigger": f"absolute training-standardized value > {args.z_threshold}",
