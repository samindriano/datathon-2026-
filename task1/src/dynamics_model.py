"""Per-road asymmetric local-dynamics ridge for Task 1."""
...
def dynamics_features(history: np.ndarray) -> np.ndarray:
    ...
    recent = history[:, -5:]
    centered = np.arange(5, dtype=np.float32) - 2.0
    slope = np.einsum("str,t->sr", recent, centered) / 10.0
    mean_recent = history[:, -3:].mean(axis=1)
    mean_middle = history[:, -6:-3].mean(axis=1)
    mean_old = history[:, -9:-6].mean(axis=1)
    curvature = (mean_recent - mean_middle) - (mean_middle - mean_old)
    mean15 = history.mean(axis=1)
    return np.stack(
        (
            history[:, -1],
            mean_recent,
            recent.mean(axis=1),
            mean15,
            np.maximum(slope, 0.0),
            np.minimum(slope, 0.0),
            curvature,
            mean_recent - mean15,
            history.max(axis=1) - history.min(axis=1),
            history.std(axis=1),
        ),
        axis=2,
...
class DynamicsRidgeModel:
    ...
    def predict(self, history: np.ndarray) -> np.ndarray:
        features = dynamics_features(history).astype(np.float64)
...
def fit_dynamics_ridge(
    ...
    for start in range(origin_start, origin_end + 1, chunk_size):
        ...
        x = dynamics_features(histories).astype(np.float64)
