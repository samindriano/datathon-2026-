"""Run the fixed rank-4 Task 1 latent spatial-factor experiment."""
...
def acceptance_gate(candidates: dict, stress: dict, diagnostics: dict) -> dict:
    ...
    candidate = candidates["lowrankblend"]
    ...
    checks = {
        ...
        <= 0.99 * float(reference["mean_mse"]),
        "all_three_folds_improve": int((candidate_folds < reference_folds).sum()) == 3,
        ...
        "at_least_17_of_18_cells_improve": int(
            stress["block_fold_horizon_cells_improved"]
        )
        >= 17,
        "at_least_34_of_36_chunks_improve": int(stress["temporal_chunks_improved"])
        >= 34,
        "median_chunk_gain_at_least_0_25_mse": float(
            stress["temporal_chunk_gain_median"]
        )
        >= 0.25,
        "worst_chunk_gain_not_below_minus_0_25": float(
            stress["temporal_chunk_gain_min"]
        )
        >= -0.25,
        "test_m2_correction_is_nonpositive": float(
            diagnostics["lowrankblend_m2_mean_correction_vs_ridge"]
        )
        <= 0.0,
        "test_change_vs_e013_is_conservative": float(
            diagnostics["lowrankblend_vs_e013_rms"]
        )
        <= 1.0,
...
def evaluate_candidates(blocks, text_blocks, adjacency, fold_sets, args):
    methods = ("stableblend", "lowrankstate", "lowrankblend")
    ...
    for block, texts, folds in zip(blocks, text_blocks, fold_sets, strict=True):
        ...
        for fold in folds:
            ...
            graph_model = fit_graph_ridge(
                block, adjacency, fold.train_origin_start, fold.train_origin_end,
            ...
            lowrank_model = fit_lowrank_state_ridge(
                block, fold.train_origin_start, fold.train_origin_end,
                alpha=args.alpha, rank=args.rank,
                pca_sample_count=args.pca_samples, chunk_size=args.chunk_size,
            )
            text_model = fit_text_zguard(
                block, texts, fold.train_origin_start, fold.train_origin_end,
                base_alpha=args.alpha, residual_alpha=args.text_residual_alpha,
            ...
            lowrank_prediction = lowrank_model.predict(histories)
            candidate_prediction = stable_blend_predictions(
                anchor_prediction, lowrank_prediction
            )
            predictions = {
                "stableblend": e013_prediction,
                "lowrankstate": lowrank_prediction,
                "lowrankblend": candidate_prediction,
            }
            block_pca_ranges.append(
                [int(lowrank_model.pca_rows.min()), int(lowrank_model.pca_rows.max())]
            )
        ...
        pca_row_ranges.append(block_pca_ranges)
        for method in methods:
    ...
    candidate_cells = np.asarray(candidates["lowrankblend"]["block_fold_horizon_mse"])
...
def main() -> int:
    ...
    fold_sets = [
        build_folds(
            len(block), block_index=index, fold_count=args.fold_count,
    ...
    global_models = [
        fit_global_state_ridge(block, 14, len(block) - int(HORIZONS.max()) - 1,
                               alpha=args.alpha, chunk_size=args.chunk_size)
        for block in blocks
    ]
    lowrank_models = [
        fit_lowrank_state_ridge(
            block, 14, len(block) - int(HORIZONS.max()) - 1,
            alpha=args.alpha, rank=args.rank,
            pca_sample_count=args.pca_samples, chunk_size=args.chunk_size,
        )
        for block in blocks
    ...
    text_models = [
        fit_text_zguard(
            block, texts, 14, len(block) - int(HORIZONS.max()) - 1,
            base_alpha=args.alpha, residual_alpha=args.text_residual_alpha,
            z_threshold=args.text_z_threshold, chunk_size=args.chunk_size,
        )
    ...
    lowrank_prediction = np.empty(shape, dtype=np.float32)
    for index, (graph_model, global_model, lowrank_model, text_model) in enumerate(
        zip(graph_models, global_models, lowrank_models, text_models, strict=True)
    ):
        mask = regimes == index
        ...
        lowrank_prediction[mask] = lowrank_model.predict(histories)
    e013_prediction = stable_blend_predictions(anchor_prediction, global_prediction)
    candidate_prediction = stable_blend_predictions(anchor_prediction, lowrank_prediction)
    ...
    diagnostics = {
        "lowrankblend_m2_mean_correction_vs_ridge": float(
            (candidate_prediction[m2] - ridge_prediction[m2]).mean()
        ),
        "lowrankblend_vs_e013_rms": float(np.sqrt(np.mean(np.square(delta)))),
        "lowrankblend_vs_e013_mean": float(delta.mean()),
        "lowrankblend_vs_e013_min": float(delta.min()),
        "lowrankblend_vs_e013_max": float(delta.max()),
    ...
    summary = {
        ...
        "hypothesis": "Training-only rank-4 spatial factors add coordinated road-network state beyond e013 global mean under the unchanged 75:25 anchor blend.",
        "preregistered_parameters": {
            ...
            "pca_sampling": "evenly spaced causal training-history rows",
            "alpha": args.alpha,
            "temporal_chunk_size_origins": TEMPORAL_CHUNK_SIZE,
            "seed": None,
        },
