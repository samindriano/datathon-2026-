"""Run the fixed graphres/textzguard blend on frozen Task 1 folds."""
...
def acceptance_gate(candidates, diagnostics):
    graph = candidates["graphres"]
    text = candidates["textzguard"]
    blend = candidates["graphtextblend"]
    graph_folds = np.asarray(graph["fold_mse"], dtype=np.float64)
    blend_folds = np.asarray(blend["fold_mse"], dtype=np.float64)
    graph_horizons = np.asarray(list(graph["mse_by_horizon"].values()))
    blend_horizons = np.asarray(list(blend["mse_by_horizon"].values()))
    checks = {
        "mean_improves_graphres_by_at_least_0_2_percent": float(blend["mean_mse"])
        <= 0.998 * float(graph["mean_mse"]),
        "improves_at_least_two_folds_vs_graphres": int(
            (blend_folds < graph_folds).sum()
        )
        >= 2,
        "improves_at_least_two_horizons_vs_graphres": int(
            (blend_horizons < graph_horizons).sum()
        )
        >= 2,
        "worst_fold_not_worse_than_graphres": float(blend["worst_fold_mse"])
        <= float(graph["worst_fold_mse"]),
        "mean_beats_graphres": float(blend["mean_mse"]) < float(graph["mean_mse"]),
        "mean_beats_textzguard": float(blend["mean_mse"])
        < float(text["mean_mse"]),
...
def evaluate_candidates(blocks, text_blocks, adjacency, fold_sets, args):
    methods = ("ridge-history", "graphres", "textzguard", "graphtextblend")
    ...
    for block, texts, folds in zip(blocks, text_blocks, fold_sets, strict=True):
        ...
        for fold in folds:
            ...
            block_guard_counts.append(
                int(text_model.guard_mask(validation_texts).sum())
            )
...
def main() -> int:
    ...
    graph_prediction = np.empty(shape, dtype=np.float32)
    text_prediction = np.empty(shape, dtype=np.float32)
    guard_counts = {}
    for regime_index, (graph_model, text_model) in enumerate(
        zip(graph_models, text_models, strict=True)
    ):
    ...
    blend_prediction = blend_predictions(graph_prediction, text_prediction)

    m2 = regimes == 1
    diagnostics = {
        "graph_m2_mean_correction_vs_ridge": float(
            (graph_prediction[m2] - ridge_prediction[m2]).mean()
        ),
    ...
    summary = {
        ...
        "hypothesis": "A fixed equal average combines complementary graph and guarded-text predictions without weight tuning.",
