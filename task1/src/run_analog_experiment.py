"""Run d1-e004-analog on the frozen Task 1 validation folds."""
...
from analog_model import FEATURE_GROUPS, fit_analog
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--selected-road-count", type=int, default=64)
    parser.add_argument("--neighbor-count", type=int, default=8)
    parser.add_argument("--delta-shrinkage", type=float, default=0.5)
...
def acceptance_gate(
    ridge: dict[str, object], analog: dict[str, object]
) -> dict[str, object]:
    ridge_folds = np.asarray(ridge["fold_mse"], dtype=np.float64)
    analog_folds = np.asarray(analog["fold_mse"], dtype=np.float64)
    ridge_horizons = np.asarray(list(ridge["mse_by_horizon"].values()))
    analog_horizons = np.asarray(list(analog["mse_by_horizon"].values()))
    checks = {
        "mean_improves_by_at_least_0_5_percent": float(analog["mean_mse"])
        <= 0.995 * float(ridge["mean_mse"]),
        "improves_at_least_two_folds": int((analog_folds < ridge_folds).sum()) >= 2,
        "improves_at_least_two_horizons": int(
            (analog_horizons < ridge_horizons).sum()
        )
        >= 2,
        "worst_fold_within_one_percent": float(analog["worst_fold_mse"])
        <= 1.01 * float(ridge["worst_fold_mse"]),
    }
    return {
        "checks": checks,
        "folds_improved": int((analog_folds < ridge_folds).sum()),
        "horizons_improved": int((analog_horizons < ridge_horizons).sum()),
...
def evaluate_candidates(
    ...
    methods = ("mean15", "ridge-history", "analog")
    ...
    for block, folds in zip(blocks, fold_sets, strict=True):
        ...
        for fold in folds:
            ...
            analog = fit_analog(
                ...
                fold.train_origin_start,
                fold.train_origin_end,
                selected_road_count=args.selected_road_count,
                neighbor_count=args.neighbor_count,
                delta_shrinkage=args.delta_shrinkage,
            ...
            predictions = {
                ...
                "analog": analog.predict(histories),
            }
...
def main() -> int:
    ...
    candidates = evaluate_candidates(blocks, fold_sets, args)
    gate = acceptance_gate(candidates["ridge-history"], candidates["analog"])
    ...
    summary = {
        ...
        "hypothesis": "Nearest causal network states provide road-level future deltas that improve over per-road ridge summaries.",
        "preregistered_parameters": {
            "feature_groups": list(FEATURE_GROUPS),
            "selected_road_count": args.selected_road_count,
            "road_selection": "highest training-origin last-speed variance",
            "neighbor_count": args.neighbor_count,
            "delta_shrinkage": args.delta_shrinkage,
            "ridge_reference_alpha": args.ridge_alpha,
            "zero_history_guard": True,
            "seed": None,
        },
        "candidates": candidates,
        "acceptance_gate": gate,
        "analog_improvement_vs_ridge_mse": float(
            candidates["ridge-history"]["mean_mse"]
            - candidates["analog"]["mean_mse"]
        ),
        "test_regime_counts": {
        ...
        "selected_roads": {
            "m1": final_models[0].selected_roads.tolist(),
            "m2": final_models[1].selected_roads.tolist(),
        },
        "candidate_origin_counts": {
            "m1": int(len(final_models[0].candidate_origins)),
            "m2": int(len(final_models[1].candidate_origins)),
        },
        "prediction": {
