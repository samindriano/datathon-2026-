"""Small per-road ridge model using only the available 15-step history."""
...
FEATURE_NAMES = ("last", "mean3", "mean5", "mean15", "slope5")


def history_features(history: np.ndarray) -> np.ndarray:
    """Return features shaped (samples, roads, features)."""
...
class RoadRidgeModel:
    ...
    def predict(self, history: np.ndarray) -> np.ndarray:
        ...
        prediction = self.target_mean[None, :, :] + np.einsum(
            "nrf,rfh->nrh", standardized, self.coefficients
        )
...
def fit_road_ridge(
    ...
    """Fit independent road coefficients from training-fold origins only."""
...
def classify_test_regime(history: np.ndarray, zero_road_threshold: int = 100) -> np.ndarray:
    """Route test samples to m1-like (0) or m2-like (1) coverage regimes."""
