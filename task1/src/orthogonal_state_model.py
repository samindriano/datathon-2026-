"""Global state plus cross-sectionally orthogonal low-rank road factors."""
...
from global_state_model import FEATURE_NAMES as GLOBAL_FEATURE_NAMES
from global_state_model import global_state_history_features
...
def fit_orthogonal_basis(
    block: np.ndarray,
    origin_end: int,
    rank: int = DEFAULT_RANK,
    sample_count: int = DEFAULT_PCA_SAMPLES,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Fit PCA after removing the training-only cross-road city level."""
    ...
    active = np.any(sample != 0, axis=0)
    if int(active.sum()) < rank:
        raise ValueError("not enough active roads for requested rank")
    ...
    city_level = standardized[:, active].mean(axis=1, keepdims=True)
    residual = np.zeros_like(standardized)
    residual[:, active] = standardized[:, active] - city_level
    _, _, right = np.linalg.svd(residual, full_matrices=False)
    basis = right[:rank].astype(np.float64, copy=False)
    return road_mean, road_scale, active, basis, rows


def orthogonal_state_history_features(
    history: np.ndarray,
    ...
    """Combine explicit city summaries with residual spatial-factor summaries."""
    history = np.asarray(history, dtype=np.float32)
    global_features = global_state_history_features(history)
    ...
    if active.shape != road_mean.shape or basis.shape[1] != history.shape[2]:
        raise ValueError("active mask or basis shape does not match history")
    standardized = (history.astype(np.float64) - road_mean[None, None, :]) / road_scale[
        None, None, :
    ]
    city_level = standardized[:, :, active].mean(axis=2, keepdims=True)
    residual = np.zeros_like(standardized)
    residual[:, :, active] = standardized[:, :, active] - city_level
    factors = np.einsum("ntr,kr->ntk", residual, basis, optimize=True)
    factor_features = history_features(factors.astype(np.float32)).reshape(len(history), -1)
    factor_features = np.broadcast_to(
        factor_features[:, None, :],
        (len(history), history.shape[2], factor_features.shape[1]),
    )
    return np.concatenate((global_features, factor_features), axis=2).astype(
        np.float32, copy=False
    )


@dataclass(frozen=True)
class OrthogonalStateRidgeModel:
    ...
    def predict(self, history: np.ndarray) -> np.ndarray:
        features = orthogonal_state_history_features(
            history, self.road_mean, self.road_scale, self.active, self.basis
        ).astype(np.float64)
...
def fit_orthogonal_state_ridge(
    block: np.ndarray,
    origin_start: int,
    origin_end: int,
    alpha: float = 0.1,
    ...
    road_mean, road_scale, active, basis, pca_rows = fit_orthogonal_basis(
        block, origin_end, rank=rank, sample_count=pca_sample_count
    )
    road_count = block.shape[1]
    feature_count = len(GLOBAL_FEATURE_NAMES) + rank * len(LOCAL_FEATURE_NAMES)
    ...
    for start in range(origin_start, origin_end + 1, chunk_size):
        ...
        x = orthogonal_state_history_features(
            histories, road_mean, road_scale, active, basis
        ).astype(np.float64)
    ...
    return OrthogonalStateRidgeModel(
