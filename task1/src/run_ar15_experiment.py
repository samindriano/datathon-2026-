"""Run d1-e005-ar15 on the frozen Task 1 validation folds."""
...
from ar15_model import FEATURE_NAMES, fit_road_ar15
...
def acceptance_gate(
    ridge: dict[str, object], ar15: dict[str, object]
) -> dict[str, object]:
    ridge_folds = np.asarray(ridge["fold_mse"], dtype=np.float64)
    ar15_folds = np.asarray(ar15["fold_mse"], dtype=np.float64)
    ridge_horizons = np.asarray(list(ridge["mse_by_horizon"].values()))
    ar15_horizons = np.asarray(list(ar15["mse_by_horizon"].values()))
    checks = {
        "mean_improves_by_at_least_0_5_percent": float(ar15["mean_mse"])
        <= 0.995 * float(ridge["mean_mse"]),
        "improves_at_least_two_folds": int((ar15_folds < ridge_folds).sum()) >= 2,
        "improves_at_least_two_horizons": int(
            (ar15_horizons < ridge_horizons).sum()
        )
        >= 2,
        "worst_fold_within_one_percent": float(ar15["worst_fold_mse"])
        <= 1.01 * float(ridge["worst_fold_mse"]),
    }
    return {
        "checks": checks,
        "folds_improved": int((ar15_folds < ridge_folds).sum()),
        "horizons_improved": int((ar15_horizons < ridge_horizons).sum()),
        "status": "KEEP" if all(checks.values()) else "REJECT",
    }


def evaluate_candidates(
    blocks: list[np.ndarray], fold_sets: list[list], alpha: float, chunk_size: int
) -> dict[str, dict[str, object]]:
    methods = ("mean15", "ridge-history", "ar15")
...
def main() -> int:
    ...
    candidates = evaluate_candidates(
        blocks, fold_sets, alpha=args.alpha, chunk_size=args.chunk_size
    )
    gate = acceptance_gate(candidates["ridge-history"], candidates["ar15"])
    ...
    summary = {
        ...
        "hypothesis": "All 15 road-specific causal lags retain useful dynamics lost by five handcrafted ridge summaries.",
        ...
        "ar15_improvement_vs_ridge_mse": float(
            candidates["ridge-history"]["mean_mse"]
            - candidates["ar15"]["mean_mse"]
        ),
        "test_regime_counts": {
