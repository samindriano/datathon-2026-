"""Per-road ridge augmented with causal citywide speed-distribution summaries."""
...
STATE_SERIES = ("city_mean", "city_std", "fraction_below_30", "fraction_below_50")
FEATURE_NAMES = (
    *LOCAL_FEATURE_NAMES,
    *(f"{series}_{name}" for series in STATE_SERIES for name in LOCAL_FEATURE_NAMES),
)


def distribution_state_history_features(history: np.ndarray) -> np.ndarray:
    """Append active-road city mean, dispersion, and slow-fraction histories."""
    history = np.asarray(history, dtype=np.float32)
    local = history_features(history)
    active = np.any(history != 0, axis=1)
    count = np.maximum(active.sum(axis=1, keepdims=True), 1)
    active3 = active[:, None, :]
    masked = history * active3
    city_mean = masked.sum(axis=2) / count
    second_moment = np.square(masked).sum(axis=2) / count
    city_std = np.sqrt(np.maximum(second_moment - np.square(city_mean), 0.0))
    fraction_below_30 = ((history < 30.0) & active3).sum(axis=2) / count
    fraction_below_50 = ((history < 50.0) & active3).sum(axis=2) / count
    state_features = []
    for series in (city_mean, city_std, fraction_below_30, fraction_below_50):
        state_features.append(history_features(series[:, :, None])[:, 0, :])
    state = np.concatenate(state_features, axis=1)
    state = np.broadcast_to(state[:, None, :], (len(history), history.shape[2], 20))
    return np.concatenate((local, state), axis=2).astype(np.float32, copy=False)


@dataclass(frozen=True)
class DistributionStateRidgeModel:
    ...
    def predict(self, history: np.ndarray) -> np.ndarray:
        features = distribution_state_history_features(history).astype(np.float64)
...
def fit_distribution_state_ridge(
    block: np.ndarray,
    origin_start: int,
    origin_end: int,
    alpha: float = 0.1,
    chunk_size: int = 256,
) -> DistributionStateRidgeModel:
    """Fit per-road ridge using local and city-distribution state summaries."""
    ...
    for start in range(origin_start, origin_end + 1, chunk_size):
        ...
        x = distribution_state_history_features(histories).astype(np.float64)
    ...
    return DistributionStateRidgeModel(
