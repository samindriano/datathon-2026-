"""Run d1-e003-lagblend on the frozen Task 1 validation folds."""
...
from lagblend_model import fit_lagblend
...
EXPERIMENT_ID = "d1-e003-lagblend"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lag-alpha", type=float, default=1.0)
...
def evaluate_candidates(
    ...
    methods = ("mean15", "ridge-history", "lagblend")
    ...
    for block, folds in zip(blocks, fold_sets, strict=True):
        ...
        for fold in folds:
            ...
            predictions = {
                ...
                "ridge-history": models["ridge-history"].predict(histories),
                "lagblend": models["lagblend"].predict(histories),
            }
...
def acceptance_gate(
    ridge: dict[str, object], lagblend: dict[str, object]
) -> dict[str, object]:
    ridge_folds = np.asarray(ridge["fold_mse"], dtype=np.float64)
    lag_folds = np.asarray(lagblend["fold_mse"], dtype=np.float64)
    ridge_horizons = np.asarray(list(ridge["mse_by_horizon"].values()))
    lag_horizons = np.asarray(list(lagblend["mse_by_horizon"].values()))
    checks = {
        "mean_improves_by_at_least_0_5_percent": float(lagblend["mean_mse"])
        <= 0.995 * float(ridge["mean_mse"]),
        "improves_at_least_two_folds": int((lag_folds < ridge_folds).sum()) >= 2,
        "improves_at_least_two_horizons": int(
            (lag_horizons < ridge_horizons).sum()
        )
        >= 2,
        "worst_fold_within_one_percent": float(lagblend["worst_fold_mse"])
        <= 1.01 * float(ridge["worst_fold_mse"]),
    }
    return {
        "checks": checks,
        "folds_improved": int((lag_folds < ridge_folds).sum()),
        "horizons_improved": int((lag_horizons < ridge_horizons).sum()),
...
def main() -> int:
    ...
    candidates = evaluate_candidates(
        blocks,
        fold_sets,
        lag_alpha=args.lag_alpha,
        ridge_alpha=args.ridge_alpha,
        chunk_size=args.chunk_size,
    )
    gate = acceptance_gate(candidates["ridge-history"], candidates["lagblend"])
    ...
    summary = {
        ...
        "hypothesis": "Shared convex 15-lag weights per regime and horizon improve robustness without per-road overfit.",
        "preregistered_parameters": {
            "lag_alpha": args.lag_alpha,
            "ridge_reference_alpha": args.ridge_alpha,
            "simplex_constraint": True,
            "weights_shared_across_roads": True,
            "weights_separate_by_regime_and_horizon": True,
            "exclude_zero_history_pairs_from_fit": True,
            "zero_history_guard": True,
            "seed": None,
        },
        "candidates": candidates,
        "acceptance_gate": gate,
        "lagblend_improvement_vs_ridge_mse": float(
            candidates["ridge-history"]["mean_mse"]
            - candidates["lagblend"]["mean_mse"]
        ),
        "test_regime_counts": {
        ...
        "final_weights": {
            "m1": final_models[0].weights.tolist(),
            "m2": final_models[1].weights.tolist(),
        },
        "prediction": {
