"""Run the frozen d2-e002-metarank experiment on official Task 2 folds."""
...
def main() -> int:
    ...
    for fold in range(args.fold_count):
        ...
        ranker = MetaRanker.fit(training, articles, categories)
        ...
        method_scores["metarank"].append(accuracy(truth, ranker_prediction))
        ...
        for subset_name, mask in (
            ...
            aggregate[subset_name]["metarank_correct"] += int(
        ...
        fold_metrics.append(
            {
                ...
                "scores": {
                    ...
                    "metarank": method_scores["metarank"][-1],
                },
            }
        )
    ...
    ranker_summary = score_summary["metarank"]
    fold_wins = int(
        sum(
            ranker > baseline
            for ranker, baseline in zip(
                method_scores["metarank"], method_scores["current_mode"], strict=True
            )
        )
    )
    subset_summary = {}
    for subset_name, values in aggregate.items():
        ...
        ranker_accuracy = aggregate_accuracy(values["metarank_correct"], values["rows"])
        subset_summary[subset_name] = {
            "rows": values["rows"],
            "current_mode_accuracy": baseline_accuracy,
            "metarank_accuracy": ranker_accuracy,
            "accuracy_delta": ranker_accuracy - baseline_accuracy,
        }

    full_ranker = MetaRanker.fit(train, articles, categories)
    ...
    gate_checks = {
        ...
        "mean_gain_at_least_0_010": ranker_summary["mean_accuracy"]
        - baseline_summary["mean_accuracy"]
        >= MINIMUM_MEAN_GAIN,
        "wins_at_least_4_of_5_folds": fold_wins >= 4,
        "worst_fold_not_worse": ranker_summary["worst_fold_accuracy"]
        >= baseline_summary["worst_fold_accuracy"],
        "current_unseen_drop_within_0_005": subset_summary["current_unseen"][
            "accuracy_delta"
        ]
        >= -MAXIMUM_SUBSET_DROP,
    ...
    summary = {
        ...
        "hypothesis": "Frozen target-aware category/title similarity selects observed next-click candidates better than current-specific frequency.",
        "model": {
            "category_weight": 2.0,
            "title_weight": 1.0,
