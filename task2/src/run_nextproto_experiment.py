...
def main():
 ...
 expected={int(f['fold']):f['target_sha256'] for f in manifest['folds']}; fs=[]; scores=[]; base_scores=[]; agg={k:[0,0,0] for k in ('current_unseen','category_entirely_unseen')}
 ...
 for f in range(a.fold_count):
  ...
  base=CurrentModeBaseline.fit(tr); model=NextProto.fit(tr,articles,cats); truth=va.next_article_id.to_numpy(np.int64); bp=base.predict(va); pp=model.predict(va,articles,cats)
  bs=accuracy(truth,bp); ps=accuracy(truth,pp); base_scores.append(bs); scores.append(ps)
 ...
 subset={k:{'rows':v[0],'current_mode_accuracy':v[1]/v[0],'nextproto_accuracy':v[2]/v[0],'accuracy_delta':(v[2]-v[1])/v[0]} for k,v in agg.items()}
 foldwins=sum(x>y for x,y in zip(scores,base_scores)); pc=pd.Series(pred).value_counts(normalize=True)
